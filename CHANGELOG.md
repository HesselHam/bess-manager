...
- Load Profile: add day-of-week labels (ma/di/wo…) to legend dates; days are now toggleable via custom legend buttons

## [7.9.120] - 2026-04-17

### Fixed

- Load Profile: fix empty chart by using correct sensor key format (`sensor.` prefix) when looking up InfluxDB results

## [7.9.119] - 2026-04-17

### Added

- Load Profile page: multi-line chart showing last 7 days of home load (W per 15-min period) overlaid on a single day axis

## [7.9.118] - 2026-04-16

### Changed

- Revert v7.9.117 (Decision Details column rename/reorder) — needs redesign.

## [7.9.116] - 2026-04-16

### Changed

- battery_system_manager.py: TOU write timing reduced from N×7s to N×2s per write, based on observed ~2s write duration.

## [7.9.115] - 2026-04-16

### Fixed

- dp_battery_algorithm.py: SOLAR_STORAGE now skipped when solar production ≤ 0.01 kWh — prevents misleading intent label when there is no solar to store.
...
- `energy_flow_calculator.py`: removed `lifetime_load_consumption` as source for
  `load_consumption`. Only `dp.load_forecast_sensor` provides `load_consumption` now,
  consistent with DP forecast behavior. If not configured, `load_consumption` is 0.0.
...
- Re-add `influxdb_21d_unique_day_avg` consumption strategy (reverted in v7.9.104 due to
  incorrect diagnosis; the sensor lookup was correct all along).
...
- Set HA API max_attempts to 1 (no retries) so failed TOU writes fail fast
  instead of blocking for up to 14 seconds per segment.
...
- HOLD mode charge_rate and discharge_rate set to 0% (was 100%/100%) to prevent accidental
  load_support behavior during BDC startup latency before hardware isolation activates.

## [7.9.96] - 2026-04-14

### Changed

- Simplified `_get_current_battery_soc`: removed unreachable `if self._controller` guard and
  redundant fallback warning — controller is always present per deterministic design principle.
...
- Add detailed logging to load sensor detection so failures are visible in logs.
...
- Fix kWh load sensor not being added to cumulative batch: detection in `__init__` ran
  before InfluxDB was reachable and silently fell back to "power". Detection now runs
  lazily on the first batch load (`_ensure_batch_data_loaded`) when InfluxDB is guaranteed
  available. On success the batch cache is cleared so the next fetch includes the sensor.

## [7.9.91] - 2026-04-14

### Changed

- Decision Details table no longer auto-refreshes every 60 seconds; manual refresh button remains functional.

## [7.9.90] - 2026-04-14

### Fixed

- Decision Details actual Verbruik now uses `local_load_power` kWh sensor (e.g.
  `total_home_load_energy`) as primary `load_consumption` source when it is detected as a
  cumulative energy sensor at startup. This replaces `lifetime_load_consumption` (0.1 kWh
  resolution) with a high-resolution delta. Replaces the broken override approach from
  v7.9.88–v7.9.89 with a clean cumulative-sensor integration.

## [7.9.87] - 2026-04-13

### Added

- `influxdb_7d_avg` consumption strategy now supports cumulative energy sensors (kWh)
  in addition to power sensors (W). Sensor type is auto-detected via a single InfluxDB
  query; both types can be set as `local_load_power` without any config change.
...
- Modbus TOU control: entity names were built with double underscore when
  `modbus_tou_entity_prefix` had a trailing underscore (e.g. `growatt_modbus_min3600tlxh_`
  produced `growatt_modbus_min3600tlxh__time_1_active`). Fixed by stripping trailing
  underscore from prefix before building entity IDs. Updated config example and docstring
  to show correct prefix format (`growatt_modbus`).
...
- Load segments: segment average was computed from actual measured consumption for past
  periods instead of the 7-day forecast profile, causing the averaged value to be much
  lower than intended. Fixed by using the pure forecast as the averaging reference.
...
- Bump version for HA update detection.
...
- DP consumption input rounded up to nearest 0.025 kWh before optimization.
  Smooths out small forecast noise (e.g. 0.086 vs 0.093 kWh) that caused the
  V-matrix to make different decisions for effectively identical consumption levels.

## [7.9.70] - 2026-04-09

### Added

- `battery.dp_soe_states` config option (default 100): controls the number of
  discrete SOE states in the DP optimizer. Higher values improve accuracy at
  the cost of proportionally more CPU time. Configurable per installation.

## [7.9.69] - 2026-04-09

### Fixed

- DP optimizer: replace fixed `SOE_STEP_KWH=0.1` with `N_SOE_STATES=100` so the
  state index step is always consistent with the linspace grid. Previously, the
  usable range (6.75 kWh) divided by 0.1 gave 67.5 → 68 states with actual step
  0.09926 kWh, but next_i was computed with 0.1 — a systematic mismatch of up to
  0.087 kWh per lookup. This caused the V-matrix to return values for the wrong
  SOE state, biasing decisions (e.g. preferring export over hold+load-support).
